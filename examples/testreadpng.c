...
xfield,yfield,scale,species,rseed,border,asyn,thetime,layers,graphics,popdyn,spacetime;
 ...
 extern int *graph;
 double Uniform();
void ReadLayerPNG(char *fname, int layer);

 void InitConstants()
 {
      xfield   = 1616;	   /* horizontal size of the automaton */
      yfield   = 1080;	   /* vertical size of the automaton */
...
void InitGraphs()
{
 ...
 {
      int x,y,k;
      ReadLayerPNG("threshold.png",1);
     //   WriteLayer("out.png",0);
     InitRandomLayer(0);
