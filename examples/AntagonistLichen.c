...
extern int xfield,yfield,scale,species,rseed,border,asyn,thetime,graphics,spacetime,popdyn;
 ...
 int 	AlgalPartner[]		={2,4}, 			FungalPartner			=1,			ParasiteTarget		=1, 	ParasiteAntagonist		=4;
 ...
 double ParasiteAbundance= 0.01, AntagonistDiscount=0, ResponseCost=0.1;
...
if((thetime==100 && LateIntroduction==1) || (thetime==0 && Square==1)){
	...
	if(state[x][y]==0) {
		...
		switch(copystate){
			...
			case 1:
			r=Uniform();
				if(CountMoore(AlgalPartner[0],x,y)>0 || CountMoore(AlgalPartner[1],x,y)>0){
			...
			r=Uniform();
				...
				case 3:
			r=Uniform();
			if(CountMoore(ParasiteAntagonist,x,y)==0){
				...
				if(CountMoore(ParasiteTarget,x,y)>0){
					if(r<ParasiticGrowth*AntagonistDiscount) {
						newstate[x][y]=3;
						BooleanGrowth=1;
					}
				}
				else{
					if(r<ParasiteSoloGrowth*AntagonistDiscount){
						newstate[x][y]=3;
						BooleanGrowth=1;
					}
				}
				...
				case 4:
							r=Uniform();
				if(CountMoore(FungalPartner,x,y)>0){
					if(r<FungalSymbioticGrowth*ResponseCost) {
						newstate[x][y]=4;
						BooleanGrowth=1;
					}
				}
				else{
					if(r<FungalSoloGrowth*ResponseCost){
						newstate[x][y]=4;
						BooleanGrowth=1;
					}
				}
...
if(thetime==100000) exit(0);				
 ...
 SpatialCorrelation()
 {
	 ...
	 for(d=0; d<=yfield/2; ++d){
		 	 FILE *fp;
     char name[5000];
     sprintf(name,"%d-%d-%d-%f-%f-%f-%f-%f-%f-%f-%f-%f-%f-%f.txt",rseed, thetime, DiffusionFactorDown, AlgalSymbioticGrowth, FungalSymbioticGrowth, ParasiticGrowth, AlgalSoloGrowth, FungalSoloGrowth, ParasiteSoloGrowth, AlgalDeath, FungalDeath, ParasiteDeath, AntagonistDiscount, ResponseCost );
